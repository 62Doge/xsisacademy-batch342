...
public interface DoctorRepository extends JpaRepository<Doctor, Long> {
    ...
    @Query(value = """
            SELECT DISTINCT d.id,
                (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM MIN(tdo.start_date))) * 12 +
                (EXTRACT(MONTH FROM CURRENT_DATE) - EXTRACT(MONTH FROM MIN(tdo.start_date))) AS month_of_experience
                FROM m_doctor d
                JOIN t_doctor_office tdo ON tdo.doctor_id = d.id
                WHERE d.id = :doctor_id
                GROUP BY d.id""", nativeQuery = true)
    DoctorDetailsMOEProjectionDto getMonthOfExperience(@Param("doctor_id") Long doctorId);
